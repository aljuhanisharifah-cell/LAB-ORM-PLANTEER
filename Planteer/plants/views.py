...
def add_plant(request):
    if request.method == "POST":
        ...
        if not name:
            return render(request, 'plants/add.html', {'error': 'Name is required'})

        if not description:
            return render(request, 'plants/add.html', {'error': 'Description is required'})
...
def add_comment(request, plant_id):
    if request.method == "POST":
        ...
        name = request.POST.get('name')   # ✨ جديد
